...
def simular_lancamento(v_zero, angulo_0, altura_inicial, ace, r, A, p, c, m, dt):
    ...
    img1 = mpimg.imread('back.jpeg')  # Substitua pelo caminho da sua imagem
    img_extent = [0, 100, 0, 100]  # Defina os limites iniciais da imagem
    img_background = ax.imshow(img1, aspect='auto', extent=img_extent, alpha=0.5)
    ...
    def update(frame):
        ...
        # Atualiza a imagem de fundo para acompanhar o gráfico
        # Aqui você pode ajustar os limites da imagem conforme o movimento do projétil
        img_background.set_extent([min(pos_x) - 10, max(pos_x) + 10, 0, max(pos_y) + 10])

        return line, img_background
...
# Função para reiniciar a simulação (fechar o gráfico atual)
def reiniciar_simulacao():
    # Fechar a janela do gráfico atual
    plt.close()
...
entry_densidade.insert(0, "1.225")
entry_densidade.place(x=180, y=260)

ttk.Label(root, text="Coeficiente de Arrasto:").place(x=20, y=300)
entry_coeficiente = ttk.Entry(root)
entry_coeficiente.insert(0, "0.47")
entry_coeficiente.place(x=180, y=300)

ttk.Label(root, text="Massa da Bola (kg):").place(x=20, y=340)
entry_massa = ttk.Entry(root)
entry_massa.insert(0, "0.045")
entry_massa.place(x=180, y=340)

ttk.Label(root, text="Intervalo de Tempo (s):").place(x=20, y=380)
entry_intervalo = ttk.Entry(root)
entry_intervalo.insert(0, "0.01")
entry_intervalo.place(x=180, y=380)

# Botão para iniciar a simulação
btn_iniciar = ttk.Button(root, text="Iniciar Simulação", command=iniciar_simulacao)
btn_iniciar.place(x=150, y=420)

# Botão para reiniciar a simulação
btn_reiniciar = ttk.Button(root, text="Reiniciar", command=reiniciar_simulacao)
btn_reiniciar.place(x=150, y=460)
